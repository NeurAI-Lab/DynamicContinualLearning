...
class TensorboardLogger:
    def __init__(self, args: Namespace, setting: str,
        ...
        for a_setting in self.settings:
            ...
            self.loggers[a_setting] = SummaryWriter(
                os.path.join(args.save_path, "tensorboard_runs", a_setting, self.name),
    ...
    def log_accuracy(self, all_accs: np.ndarray, all_mean_accs: np.ndarray,
                     args: Namespace, task_number: int, metric="acc") -> None:
        """
        ...
        for setting, a_logger in self.loggers.items():
            ...
            for kk, acc in enumerate(accs):
                a_logger.add_scalar('{}_task%02d'.format(metric) % (kk + 1), acc,
                                    task_number * args.n_epochs)
            a_logger.add_scalar('{}_mean'.format(metric), mean_acc, task_number * args.n_epochs)

    def log_heatmap(self, figure, epoch: int) -> None:
        """
        :param figure:
        :param int:
        :return: None
        """
        for a_logger in self.loggers.values():
            a_logger.add_figure('hmap', figure, global_step=epoch)

    def log_loss(self, loss: float, args: Namespace, epoch: int,
                 task_number: int, iteration: int, name="Total_loss") -> None:
        """
    ...
    def log_loss_gcl(self, loss: float, iteration: int, name="Total_loss") -> None:
        """
        Logs the loss value at each iteration.
        :param loss: the loss value
        :param iteration: the current iteration
        """
        for a_logger in self.loggers.values():
            a_logger.add_scalar(name, loss, iteration)
