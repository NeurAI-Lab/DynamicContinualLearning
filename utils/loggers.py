...
class CsvLogger:
    def __init__(self, setting_str: str, dataset_str: str,
                 model_str: str, save_path: str) -> None:
        self.accs = []
    ...
    def write(self, args: Dict[str, Any]) -> None:
        ...
        csv_dir = os.path.join(self.save_path, "results", self.setting)
        create_if_not_exists(csv_dir)
        create_if_not_exists(os.path.join(csv_dir, self.dataset))
        create_if_not_exists(os.path.join(csv_dir, self.dataset, self.model))

        write_headers = False
        path = os.path.join(csv_dir, self.dataset, self.model, "mean_accs.csv")
        ...
        if self.setting == 'class-il':
            create_if_not_exists(os.path.join(self.save_path, "results", "task-il", self.dataset))
            create_if_not_exists(os.path.join(self.save_path, "results", "task-il", self.dataset, self.model))
            ...
            write_headers = False
            # path = self.save_path + "results/task-il" + "/" + self.dataset + "/" \
            #        + self.model + "/mean_accs.csv"
            path = os.path.join(self.save_path, "results", "task-il", self.dataset, self.model, "mean_accs.csv")
            if not os.path.exists(path):
                write_headers = True
            with open(path, 'a') as tmp:
                writer = csv.DictWriter(tmp, fieldnames=columns)
